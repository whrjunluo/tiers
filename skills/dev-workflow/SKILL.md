---
name: dev-workflow
description: 开发需求工作流路由指南。当用户提出新的开发需求、功能或修复时调用，用于确定应走哪条工作流（L0 gstack / L1 SDD / L2 轻量 spec / L3 调试 / L4 直接写）并执行对应的 skill 链。
---
...
```
${CLAUDE_PLUGIN_ROOT}/scripts/codegraph-judge.sh [--repo <repo>] [--base <base>] assess
```
...
```
${CLAUDE_PLUGIN_ROOT}/scripts/learnings.sh add 判级/图风险背离 <project> "<note>"
```
...
**执行步骤：**
1. `brainstorming` skill — 澄清需求，提出 2-3 方案，用户确认
2. `writing-plans` skill — 写 spec 到 `docs/superpowers/specs/`，写 plan 到 `docs/superpowers/plans/`
3. `test-driven-development` skill — 先写失败测试，再实现，测试通过后提交
4. `requesting-code-review` skill — 请求代码审查
5. `verification-before-completion` skill — 验证功能符合 spec 后关闭任务
...
**维护约定：**
1. **开工时**：跑 `${CLAUDE_PLUGIN_ROOT}/scripts/workflow-state.sh check`，再 `get phase` / `get task` / `get next`。若 phase != done 且非空，提示续行：「检测到未完成的 {level} 任务【{task}】，停在 {phase}，下一步 {next}。继续，还是换新任务？」
...
配套工具（均在 `${CLAUDE_PLUGIN_ROOT}/scripts/`）：
...
每次本 skill 被调用，作为「开工前」第 1 步：跑 `${CLAUDE_PLUGIN_ROOT}/scripts/learnings.sh ready`（等价于按 `category` 数 `pending`）→ 任一 category ≥ 2 即输出提案，否则沉默：
