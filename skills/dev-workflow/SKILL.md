...
**任务范围校准：** 上表阈值只适用于本任务的快照。运行前优先使用 `execution.base_revision` / 本任务 checkpoint 作为 `--base`；若报告混入上一任务或用户已有脏工作区改动，先用正确 base 重跑，或人工列出本任务文件、真实 affected flow 与相关 test gap 后再判级。禁止仅凭混入无关改动后的总文件数或 risk 分数机械升 L1，也禁止因图风险接近 0 降级真实的 auth / API / IM 等业务链风险。无法隔离时按任务自身性质和已证实影响面偏严判定，并把 codegraph 范围污染记为降级证据。

**图风险背离固化规则：** codegraph 只用于校准当前任务的代码影响面，不能替代业务风险判断。若快照混入历史提交或脏改动，必须先锁定本任务 base 重跑；仍无法隔离时，改用人工影响面审查并记录降级原因。无论图风险多低，都不得据此降级 API、IM、订单等高风险业务闭环；反之，也不得因无关历史改动抬高图风险而机械升级。
...
5. **计划后的执行方式选择（一次）** — plan 完成、理解度通过后、开始任何 TDD 测试之前，host 向用户展示 `single` 与 `multi-agent`，给出推荐和可核查依据；初始选择只能在 plan 阶段进行一次，使用 `workflow-state.sh choose-execution <single|multi-agent> [manifest]` 记录。小任务、写入集有重叠、或不能明确隔离时默认并推荐 `single`；`multi-agent` 仅在并行收益明确、每个 worker 都有不重叠的 write set、并且 manifest 已绑定当前 plan 时可选。
6. **multi-agent 交接与收尾** — 共享 plan、接口说明、只读证据和审查结论是 read-only；worker 不得写主控 worktree 或 controller state，每个 worker 只能在自己的隔离 worktree 写入其 manifest 指定的 write set。host 独占主控 worktree、controller、合并与集成验证：按顺序接收 worker 产物，处理 conflict，运行测试并确认集成结果。worker 失败、超时、写入集扩大或出现无法安全解决的 conflict 时，host 记录原因，并且仅可在 `phase=tdd` 使用 `workflow-state.sh choose-execution single "<fallback reason>"` 回退为 inline 执行；这个记录的非空 fallback reason 是初始一次选择之外唯一的例外，不把失败任务交给其他 worker 继续写。cleanup only after integration verification：只在 host 已验证集成成功后 cleanup worktree、临时分支和 worker 资源；不得在 cleanup 前假定结果已集成。
...
3. bug 写一个能复现问题的失败测试；隔离新增先写覆盖显式入口、旧路由不变和能力边界的失败契约测试（先红）。
4. 修复，让测试变绿
