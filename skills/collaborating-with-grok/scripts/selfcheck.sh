#!/usr/bin/env bash
# Self-check for the collaborating-with-grok skill.
# Verifies: grok CLI on PATH, and a live one-shot round-trip through
# grok_bridge.py returns clean JSON (this also confirms grok is authed).
#
# Usage: scripts/selfcheck.sh
# Exit codes: 0 ok | 1 grok missing | 3 round-trip failed
set -uo pipefail
...
if ! command -v grok >/dev/null 2>&1; then
  say "✗ grok CLI not found on PATH. Install Grok CLI and retry."
  exit 1
fi
say "✓ grok found: $(command -v grok)"

TMP="$(mktemp -d)"; trap 'rm -rf "$TMP"' EXIT
say "→ round-trip probe (reply 'pong')..."
OUT="$(python3 "$HERE/grok_bridge.py" --cd "$TMP" \
...
if printf '%s' "$OUT" | python3 -c "import sys,json; d=json.load(sys.stdin); sys.exit(0 if d.get('success') and d.get('SESSION_ID') and d.get('agent_messages') else 1)" 2>/dev/null; then
  ...
  say "✗ round-trip failed (is grok authed? \`grok login\`). Bridge output:"
  printf '%s\n' "$OUT"
  exit 3
fi
