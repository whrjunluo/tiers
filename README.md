...
分级开发工作流路由插件，优先兼容 Codex，同时保留 Claude Code 可用的目录与 hook 配置。根据需求复杂度（L0-L4）自动路由到对应工作流，减少判级歧义、漏测风险和跨 session 进度丢失。
...
```bash
git clone <your-repo-url> ~/gst-workspace/dev-workflow-plugin
cd ~/gst-workspace/dev-workflow-plugin
...
2. 启用插件配置：`[plugins."dev-workflow-plugin@local"] enabled = true`
...
如果已经把仓库 clone 到 `~/gst-workspace/dev-workflow-plugin`，常用命令是：

```bash
~/gst-workspace/dev-workflow-plugin/bin/init --yes
```
...
grep -A1 'plugins."dev-workflow-plugin@local"' ~/.codex/config.toml
```
