...
> 按风险与验证深度给开发任务分级（L0–L4）并路由到对应工作流的 Claude Code / Codex 插件。Claude Code 里通过 `/` 菜单使用；Codex 里通过自然语言触发，或在请求里明确说 `dev-workflow`。
...
装好后无需手动跑脚本。新开会话，直接描述你的开发需求即可；如果想强制触发，就在请求里点名 `dev-workflow`：
